...
namespace DuplicatesFinder
{
    ...
    {
        internal static bool RemoveFile(string path, bool toRecycleBin)
        {
            try
            {
                if (!File.Exists(path))
                ...
                if (toRecycleBin)
                {
                    return SendToRecycleBin(path); // it doesn't support "\\?\"
                }
                if (!path.StartsWith(@"\\?\"))
                {
                    path = @"\\?\" + path;
            ...
            {
                Globals.ReportError(ex, false, "Failed to remove file: " + path);
                return false;
            }
        }
        ...
        /// <summary>
        /// Send file to recycle bin, failed if files are too big to fit
        /// </summary>
